...
async function run() {
  try {
    ...
    if (!payment) {
      console.log("No payments found in DB to test.");
      process.exit(0);
    }

    console.log(`Found payment ${payment._id} for client: ${payment.clientName}, project: ${payment.projectName}`);

    // Re-run the generation logic exactly as defined in payments.js
    ...
    const brandLogoHtml = brandLogoPath
      ...
      : `<div class="brand-logo">${adminLogoText.charAt(0).toUpperCase()}</div>`;
    ...
    const invoicesDir = path.join(__dirname, '../backend/uploads/invoices');
